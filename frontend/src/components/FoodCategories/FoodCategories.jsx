...
const FoodCategories = () => {
  ...
  useEffect(()=>{
   fetch('http://10.166.233.193:3000/categories')
   .then(res=>res.json())
   .then(data=>{
...
<div className="boxcontainer items-container flex flex-wrap gap-4" style={{marginLeft:'2%',marginTop:'1%'}}>
  {category?.items?.map(item => (
    <div key={item._id} style={{width:'23vw',height:'50vh',boxShadow:'2px 2px 2px 2px lightgray'}} className="food-card rounded-2xl">
      ...
      <button className='bg-black text-white' style={{padding:'2px 17px',marginLeft:'26%',borderRadius:'20px',marginTop:'1%'}}>{item.button}</button>
